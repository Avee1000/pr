...
export default function NotFound() {
    return (
        <div className="relative flex min-h-screen flex-col overflow-hidden text-ink font-sans">
            ...
            <main className="relative z-10 mx-auto flex w-full max-w-280 flex-1 flex-wrap items-center justify-between gap-12 px-12 pt-10 pb-24">
                ...
                <div className="max-w-120 flex-1 basis-105">
                    <div className="mb-5 inline-flex items-center gap-2 text-xs font-semibold tracking-wider text-ink/55 dark:text-muted-foreground uppercase">
                        <span className="inline-block h-1.5 w-1.5 rounded-full bg-action" />
                        Error 404
                    </div>

                    <h1 className="font-heading text-[clamp(40px,5vw,56px)] font-bold leading-none tracking-tight text-ink dark:text-muted-foreground">
                        Wrong turn
                        <br />
                        somewhere back there.
                    </h1>

                    <p className="mt-5 mb-9 max-w-100 text-base leading-relaxed text-ink/65 dark:text-muted-foreground">
                    ...
                    <div className="flex items-center gap-4">
                        <Link
                            href="/"
                            className="rounded-lg bg-brand px-6 py-3 text-sm font-semibold border-ink border text-ink shadow-[0_8px_20px_-8px_rgba(255,74,60,0.55)] transition-opacity hover:opacity-95"
                        >
                            Back to home
                        </Link>
                        <Link
                ...
                {/* Right Column: Signature Compass Illustration */}
            ...
            {/* Footer */}
            <footer className="relative z-10 pb-7 text-center text-xs text-ink/40">
