...
import fs from 'fs';
import { getAllCustomers, getQuoteByQuoteId } from '../models/pdf-model';

export const generateReceipt = async (req: Request<{ quoteId: string }>, res: Response): Promise<void> => {
  ...
  try {
    ...
    const quote = await getQuoteByQuoteId(quoteId, token);
    console.log(quote);
    ...
    doc.fontSize(20)
      .font('Helvetica-Bold')
      .fillColor('#FFFFFF')
      .text(formattedPrice, priceBoxX, totalCardCenterY, { // Use totalCardCenterY
        width: priceBoxWidth,
        align: 'right',
        baseline: 'middle'
      });
...
export const generateAll = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    const customers = await getAllCustomers(userId, accessToken);
